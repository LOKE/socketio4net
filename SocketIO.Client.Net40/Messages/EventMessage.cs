        ...
        public static EventMessage Deserialize(string rawMessage)
        {
            ...
            try
            {
                ...
                if (args.Length == 4)
                {
                    string ack = args[1];
                    int id;
                    if (int.TryParse(ack, out id))
                        evtMsg.AckId = id;

                    // is this an event that expects a user reply?
                    if (ack.Length > 0 && ack.Substring(ack.Length - 1, 1) == "+")
                    {
                        if (int.TryParse(ack.Substring(0, ack.Length - 1), out id)) evtMsg.ReplyId = id;
                    }
